...
# Modern Kaldi training script with Vosk-style TDNN chain model
...
if [ $stage -le 1 ]; then
  ...
  local/train_irstlm.sh data/local/transcript_lm/transcripts.uniq 4 "4g" data/lang data/local/train4_lm \
    &> data/local/4g.log || exit 1
fi
