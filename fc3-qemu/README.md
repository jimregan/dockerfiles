...
The plugin loading is not enough on its own for `package require snack` to succeed inside it. Every `.tcl` embedded in a page runs in its own `::safe::interpCreate`'d slave (`browser.tcl`), whose `-accessPath` is hardcoded to just `$tcl_library` and the plugin's own install directory — `/usr/lib/snack2.2` is never on it, and no amount of `TCLLIBPATH`/`auto_path` set in the master (Mozilla) process changes that, since the slave's access path is a completely separate, more restrictive mechanism from the master interpreter's `auto_path`. `TCLLIBPATH=/usr/lib/snack2.2`, exported before Mozilla launches in `/root/.xinitrc`, is still needed — but only to make the *master* interpreter's `auto_path` correct so that it can be copied into a slave once trusted, not to fix the slave directly.

Getting a slave promoted out of that sandbox requires it to request the `trusted` policy — `catch { policy trusted }`, called before `package require snack` — which triggers `unsafe.tcl`'s `InstallFeatures`: it copies `auto_path`/`tcl_library`/`tk_library` from master into the slave and restores hidden commands like `load` (needed for `libsnack.so`). Two things block this by default: tclplugin's own `config/plugin.cfg` has `disallow trusted` in `section policies` (patched to `allow trusted` in `kickstart.cfg`'s `%post` — safe here since every tclet this guest ever loads is our own, served from the walled-off `10.0.2.2` gateway), and not every `lab/*.tcl` demo actually calls `policy trusted` — `ex.tcl`/`liulab.tcl` already did, `sect.tcl`/`speg.tcl`/`fftwin.tcl`/`setgain.tcl` didn't and needed the same line added.
