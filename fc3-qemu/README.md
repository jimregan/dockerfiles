...
Loading the plugin is not enough on its own for `package require snack` to succeed inside it, and it's not a Safe Base sandboxing problem to work around — the plugin's own docs (`doc/oldFAQ`, Q17) say plainly that compiled extensions like Snack cannot be loaded into the plugin's in-process interpreter at all, safe or trusted, because `libsnack.so` was never built with the `Snack_SafeInit` entry point that loading into the Safe Base requires. `ex.tcl`'s tclet already called `catch { policy trusted }` before any of the fixes below, and still failed with the same "can't find package snack" error, confirming the trusted-policy route is a dead end here.

The plugin's supported way to use an extension is `TCL_PLUGIN_WISH` (`doc/oldFAQ`, Q18): with it set, `plugmain.tcl` spawns a real external `wish` process per tclet (via `remoted.tcl`, over a pipe) and embeds its window in the page, instead of sourcing the tclet in-process. `/root/.xinitrc` sets `TCL_PLUGIN_WISH=/usr/bin/wish` before launching Mozilla, pinning it to a known-good `wish` rather than leaving it to `plugmain.tcl`'s own fallback (on Unix it already defaults to trying an external `wish` via `plugin(executable)`, which may not resolve to anything valid). Because that external `wish` is fed the tclet with no script argument over the pipe, Tcl treats it as an interactive session and sources `~/.wishrc` on startup — `/root/.wishrc` uses that to `lappend auto_path /usr/lib/snack2.2`, which is what actually makes `package require snack` succeed.
