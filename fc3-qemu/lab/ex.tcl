...
set v(y1) [canvas .f1.f.fc.cy -width 30 -height $v(spegh) -highlightthi 0]
pack $v(y1) -si left -anchor n
set v(c1) [canvas .f1.f.fc.c -width 600 -height $v(spegh) -xscrollcom [list .f1.f.fc.xscroll set] -highlightthi 0 -closeenough 2 -cursor crosshair]
scrollbar .f1.f.fc.xscroll -orient horizontal -com [list $v(c1) xview]
pack .f1.f.fc.xscroll -si bottom -fill x
pack $v(c1) -si left
pack .f1.f -si right
...
snack_yAxis $v(y1) 0 0 30 $v(spegh) -topfrequency $v(topfr1)
snack_tAxis $v(c1) 0 [expr $v(spegh)-$v(timeh)] 600 $v(timeh) $v(pps1) 10 -tags axis -fill red
...
if {$embed_args(conf) == 1} {
    ...
    pack [ set v(c1s) [canvas $sf.c1s -width $v(sectw) -height $v(secth) -cursor crosshair -highlightthi 0]]
    bind $v(c1s) <Motion> { PutCrossHairs $v(c1s) %x %y }
    $v(c1s) create section 0 0 -sound s1 -width $v(sectw) -height $v(secth) -tags s1 -frame yes -fftlen 1024 -winlen 1024
...
if {$embed_args(conf) >= 2} {
    ...
    set v(y2) [canvas .f2.f.fc.cy -width 30 -height $v(spegh) -highlightthi 0]
    pack $v(y2) -si left -anchor n

    set v(c2) [canvas .f2.f.fc.c -width 600 -height $v(spegh) -xscrollcom [list .f2.f.fc.xscroll set] -highlightthi 0 -closeenough 2 -cursor crosshair]
    scrollbar .f2.f.fc.xscroll -orient horizontal -com [list $v(c2) xview]
    pack .f2.f.fc.xscroll -si bottom -fill x
    pack $v(c2) -si left
    pack .f2.f -si right
    if {$embed_args(conf) == 2} {
    ...
    if {$embed_args(conf) == 2} {
	snack_yAxis $v(y2) 0 0 30 $v(spegh) -topfrequency $v(topfr2)
    }
...
pack [ frame .f] -fill x
pack [ spectrogram .f.a -height 110 -width 400 -sound s0] -si left
pack [ frame .f.rp] -si left
pack [ button .f.rp.r -text Record -fg red] -si top
bind .f.rp.r <ButtonPress-1>   {s0 record -in $v(in);.f.rp.r config -activeba red}
bind .f.rp.r <ButtonRelease-1> {s0 stop;.f.rp.r config -activeba #ececec}
pack [ button .f.rp.p -text Play -fg green4 -wi 6 -com {s0 play -out $v(out)}] -si top

if {$tcl_platform(platform) != "windows"} {
    if {$tcl_platform(byteOrder) == "bigEndian"} {
	pack [ frame .f.rp.in]
	pack [ radiobutton .f.rp.in.oe -text "ext" -value Headphones -var v(out) -width 2 -font {Helvetica 9 bold}] -si left
	pack [ radiobutton .f.rp.in.oi -text "int" -value Speaker -var v(out) -width 2 -font {Helvetica 9 bold}] -si left
    }
    pack [ frame .f.rp.out]
    pack [ radiobutton .f.rp.out.oe -text "mic" -value Microphone -var v(in) -width 2 -font {Helvetica 9 bold}] -si left
    pack [ radiobutton .f.rp.out.oi -text "line" -value Line -var v(in) -width 2 -font {Helvetica 9 bold}] -si left
}
pack [ frame .f.f] -si left
pack [ frame .f.f.u -highlightthi 0] -si top
pack [ frame .f.f.u.u] -si top 
pack [ frame .f.f.u.b] -si top
pack [ button .f.f.u.u.u -text Copy -com {Copy 1}] -si left
pack [ button .f.f.u.u.pl -text Play -com { PlayMark 1 }] -si left
pack [ button .f.f.u.u.pr -text Print -com { Print 1 }]   -si left
pack [ label .f.f.u.b.l -text "analysis bw:"] -si left
pack [ entry .f.f.u.b.e -width 4 -textvar v(bw1)] -si left
pack [ label .f.f.u.b.l2 -text Hz] -si left
bind .f.f.u.b.e <Key-Return> { SetBW 1 }
pack [ frame .f.f2 -highlightthi 0] -si left -fill both
pack [ frame .f.f2.u] -si top
pack [ scale .f.f2.u.s1 -var v(pps1) -label "zoom t" -orient horiz -from 200 -to 2000 -com {ZoomT 1} -showvalue no] -si left
pack [ scale .f.f2.u.s2 -var v(fzoom1) -label "zoom f" -from 0 -to 6000 -orient hori -com {ZoomF 1} -showvalue no] -si left

if {$embed_args(conf) >= 2} {
    pack [ frame .f.f.b -highlightthi 0] -si top
    pack [ frame .f.f.b.u] -si top
    pack [ frame .f.f.b.b] -si top
    pack [ button .f.f.b.u.d -text Copy -fg red3 -com {Copy 2}] -si left
    pack [ button .f.f.b.u.pl -text Play -fg red3 -com { PlayMark 2 }] -si left
    pack [ button .f.f.b.u.pr -text Print -fg red3 -com { Print 2 }]   -si left
    if {$embed_args(conf) == 2} {
	pack [ label .f.f.b.b.l -text "analysis bw:" -fg red3] -si left
	pack [ entry .f.f.b.b.e -width 4 -textvar v(bw2) -fg red3] -si left
	pack [ label .f.f.b.b.l2 -text Hz -fg red3] -si left
	bind .f.f.b.b.e <Key-Return> { SetBW 2 }
    }
    pack [ frame .f.f2.b] -si bottom
    pack [ scale  .f.f2.b.s1 -var v(pps2) -fg red3 -label "zoom t" -orient horiz -from 200 -to 2000 -com {ZoomT 2} -showvalue no] -si left
    if {$embed_args(conf) == 2} {
	pack [ scale .f.f2.b.s2 -var v(fzoom2) -fg red3 -label "zoom f" -from 0 -to 6000 -orient hori -com {ZoomF 2} -showvalue no] -si left
    }
}
...
proc Print n {
    ...
    snack_yAxis $v(c$n) $x 0 30 $v(spegh) -topfrequency $v(topfr$n) -tags junk
...
proc Copy n {
    ...
    $v(c$n) delete axis
    snack_tAxis $v(c$n) 0 [expr $v(spegh)-$v(timeh)] $width $v(timeh) $v(pps$n) 10 -tags axis -fill red
    s$n copy s0
    set v(start$n) 0
...
proc ZoomF {n f} {
    ...
    $v(y$n) delete all
    snack_yAxis $v(y$n) 0 0 30 $v(spegh) -topfrequency $v(topfr$n)
    if {$embed_args(conf) == 1} {
